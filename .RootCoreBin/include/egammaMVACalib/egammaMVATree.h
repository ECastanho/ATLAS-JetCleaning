...
namespace egammaMVATreeHelpers
{
  ...
  inline float compute_correctedcl_Es0(const xAOD::Egamma& p) { return p.isAvailable<double>("correctedcl_Es0") ? p.auxdataConst<double>("correctedcl_Es0") : p.caloCluster()->energyBE(0); }
  inline float compute_correctedcl_Es1(const xAOD::Egamma& p) { return p.isAvailable<double>("correctedcl_Es1") ? p.auxdataConst<double>("correctedcl_Es1") : p.caloCluster()->energyBE(1); }
  inline float compute_correctedcl_Es2(const xAOD::Egamma& p) { return p.isAvailable<double>("correctedcl_Es2") ? p.auxdataConst<double>("correctedcl_Es2") : p.caloCluster()->energyBE(2); }
  inline float compute_correctedcl_Es3(const xAOD::Egamma& p) { return p.isAvailable<double>("correctedcl_Es3") ? p.auxdataConst<double>("correctedcl_Es3") : p.caloCluster()->energyBE(3); }

  inline float compute_correctedcl_Eacc(const xAOD::Egamma& p) {
    return compute_correctedcl_Es1(p) + compute_correctedcl_Es2(p) + compute_correctedcl_Es3(p);
  }

  inline float compute_correctedcl_f0(const xAOD::Egamma& p) {
    return compute_correctedcl_Es0(p) / (compute_correctedcl_Eacc(p));
  }

  inline float compute_correctedcl_calibHitsShowerDepth(const xAOD::Egamma& p) {
    const std::array<float, 4> cluster_array { compute_correctedcl_Es0(p),
	                                       compute_correctedcl_Es1(p),
                                               compute_correctedcl_Es2(p),
                                               compute_correctedcl_Es3(p) };
    return compute_calibHitsShowerDepth(cluster_array, compute_cl_eta(*p.caloCluster()));
  }
