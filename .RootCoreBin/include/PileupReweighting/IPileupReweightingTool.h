   ...
   class IPileupReweightingTool : public virtual asg::IAsgTool, virtual public CP::ISystematicsTool
{
      ...
      public:
         /// specify the setProperty methods in the interface class, so that rootcore people can work with the interface
#ifndef XAOD_STANDALONE
         virtual StatusCode setProperty( const Property& p  ) = 0;
         virtual StatusCode setProperty( const std::string& s ) = 0;
#endif
         virtual StatusCode setProperty( const std::string& propName, double val ) = 0;
         virtual StatusCode setProperty( const std::string& propName, int val ) = 0;
         virtual StatusCode setProperty( const std::string& propName, const char* val ) = 0;
         virtual StatusCode setProperty( const std::string& n, const std::string& v ) = 0;
         virtual StatusCode setProperty( const std::string& propName, const std::vector<std::string>& val  ) = 0;

#ifdef XAOD_STANDALONE
      //add way to specify GRLTool in standalone code
      virtual StatusCode setProperty( const std::string& propName, IGoodRunsListSelectionTool* grl ) = 0;
#endif
         ...
         virtual float getCorrectedMu( const xAOD::EventInfo& eventInfo, bool includedDataScaleFactor=false ) = 0;
