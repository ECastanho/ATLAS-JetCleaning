...
namespace SH
{
  ...
  struct DQ2
  {
    ...
    /// \brief list all the files in the given dataset or container
    ///   for use with FAX
    /// \pre (soft) !name.empty()
    /// \pre (soft) name.find('*') == std::string::npos
    /// \par Guarantee
    ...
    listFiles_fax (const std::string& name, const std::string& filter);


    /// \copydoc listFiles_fax
    /// \pre (soft) name.find('/') == std::string::npos
    static std::vector<std::string>
    listFiles_fax_ds (const std::string& name, const std::string& filter);
    ...
    /// \brief list all the datasets and containers matching the
    ///   pattern, skipping containers unless the pattern ends in "/"
    /// \pre (soft) !pattern.empty()
    /// \pre (soft) pattern[0] != '*'
    /// \par Guarantee
    ///   strong
    /// \par Failures
    ...
    ///   dq2 errors
    /// \par Rationale
    ///   I'm skipping containers for datasets ending in a
    ///   "*", since it is almost always a mistake to pick up
    ///   containers with a pattern that can also pick up datasets
    static std::vector<std::string>
    ls (const std::string& pattern);


    /// \brief list all the datasets and containers matching the pattern
    /// \pre (soft) !pattern.empty()
    /// \pre (soft) pattern[0] != '*'
    /// \par Guarantee
    ///   strong
    ...
    ///   dq2 errors
    static std::vector<std::string>
    ls_raw (const std::string& pattern);
